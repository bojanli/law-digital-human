...
4. 服务端执行“引用校验”：
5. 本地知识库无证据时，先声明内容来自公开网络信息并给出谨慎参考；网络不可用时给出一般性提示和追问。
...
3  引用校验与外部来源规则说明
1. 当本地检索结果为空时，服务端不再直接拒答；会尝试公开网络检索，并在回答中明确声明外部来源与免责声明。
2. 当 `citations` 为空或不在检索结果中时，服务端拒答或过滤非法引用，避免外部内容被展示为本地知识库依据。
